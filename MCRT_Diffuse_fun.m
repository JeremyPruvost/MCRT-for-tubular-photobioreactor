function [gamma, LRPA, LRPA_gamma, q_PFD] = tube_ray_tracing_fun(INPUTS,NameFileKineticsParameter)
...
theta = acosd(dot(tube,sun,2));

q_PFD = q_sol; % Case of diffuse incident light
...
if asdf == 1
    plot3([sc1(1) sc2(1)],[sc1(2) sc2(2)],[sc1(3) sc2(3)],'-r');
end
...
parfor i = 1:Nrays
    ...
    while fin == 0
        ...
        if interface == 1
            ...
            rando = rand();  
            x = r_o*cosd((2*rando-1)*90);
            y = r_o*sind((2*rando-1)*90);
            int = [x, y, 0];
            ang_rang = 180 - abs((2*rando-1)*90) - cut_off;

            if y > 0
                phi = (90-cut_off) - rand()*ang_rang;
            elseif y < 0
                phi = (-90+cut_off) + rand()*ang_rang;
            end
            emit = [1.1*r_o*cosd(phi) + int(1), 1.1*r_o*sind(phi) + int(2),0 + int(3)];
            ray1 = [int(1)-emit(1), int(2)-emit(2), int(3)-emit(3)];
            ray1 = ray1./norm(ray1);
...
%                             ray_path = [ray_path; abs_loc(i,:)];
                ...
                case 4 % tube/air
                    ...
                    interface = 0;
                        ...
                        if Ninterface>100
                            disp('Photon trapped in tube wall : destroyed')
                    %        pause(0.1)
                        fin=1;
                        end
...
disp('% of rays absorbed');disp(alpha)
disp('% of rays reflected');disp(rho)


%%%% Validation that number of complex number due to angle
...
% Calculate the growth average on the tube volume (finite values, orherwise set equal to -inf), excluding positions where no absorption occured (LRPA>0) 
...
LRPA{k}(ind_ext)=-1/0;
ind_fin=find(LRPA{k}>0); 


end

q_i = (Ntube*E_photon)/(r_i*pi);