...
% defaults. For compatibility, we set the color now, and then later set a 
% semantic variable if needed:
cc = repmat(cat(3, 0.7882, 0.9059, 1), [size(zz) 1]);
surfh = surf(cax, xx, yy, zz, cc, 'tag','hist3', plotArgs{:});
% Extract 'CDataMode' and 'FaceColor':
plnames = {'CDataMode','FaceColor'};
pldflts = {'manual','flat'};
[cdatamode,facecolor,~,~] = internal.stats.parseArgs(plnames,pldflts,plotArgs{:});
facecolorChk=strcmpi(facecolor,{'flat','interp','texturemap'});
% Only set semantic variable if we expect the default color:
if (strcmpi(cdatamode,'manual') & any(facecolorChk))
    % For compatibility reasons, we don't alter the FaceColorMode:
    facecolormode = surfh.FaceColorMode;
    matlab.graphics.internal.themes.specifyThemePropertyMappings(...
            surfh,'FaceColor',"--mw-graphics-colorOrder-1-tertiary");
    surfh.FaceColorMode = facecolormode;
end
...
if ~isdeployed
    if ~isempty(surfh)   
        % Generate code for hist3.
        setappdata(surfh(1),'inputArgs',inputArgs);
        makemcode('RegisterHandle',surfh(1),'MCodeConstructorFcn',@stats.internal.mcode.mcodeconstructorHist3,'MCodeIgnoreHandleFcn',[]);
    end
end
